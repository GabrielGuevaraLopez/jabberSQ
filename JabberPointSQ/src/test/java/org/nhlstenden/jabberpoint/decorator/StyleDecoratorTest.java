...
class StyleDecoratorTest {
  ...
  void testFontSizeDecoratorMultipleSizes() {
    ...
    assertNotEquals(decorated1.getFont(scale).getSize(), decorated2.getFont(scale).getSize());
  }

  @Test
  void testMultipleDecorators() {
    StyleComponent decorated =
        new FontSizeStyleDecorator(new ColorStyleDecorator(baseStyle, Color.RED), 48);
    assertEquals(Color.RED, decorated.getColor());
  ...
  void testDecoratorOrder() {
    StyleComponent order1 =
        new FontSizeStyleDecorator(new ColorStyleDecorator(baseStyle, Color.RED), 48);
    StyleComponent order2 =
        new ColorStyleDecorator(new FontSizeStyleDecorator(baseStyle, 48), Color.RED);
    assertEquals(order1.getColor(), order2.getColor());
    assertEquals(order1.getFont(scale).getSize(), order2.getFont(scale).getSize());
  }
